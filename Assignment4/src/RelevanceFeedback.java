...
public static void main(String[] args) throws IOException {
		...
		LinkedList<DocScore> docScoreAll;
		
		LinkedList<DocScore> docScore;
		...
		docScore = indexer.getRankedListOf(inputToken, documentsToBeRetrieved); //Getting first ranked list
		...
		for (int i = 0; i < nonRelevantSumVector.length; i++) {
			...
			relevantSumVector[i]=0; //initializing with zero
			
			nonRelevantSumVector[i]=0; //initializing with zero
			
		}
		
		Parameters parameter = new Parameters(1.0, 0.75, 0.15);	//setting the parameters of the formula for finding modified query
		...
		//Rochio mechanism starts
		
		System.out.print("Now give your document preference feedback : ");
		
		sc = new Scanner(System.in);
		
		String feedback = sc.nextLine();
		
		for(String docID : feedback.split(" ") ){
			
			relevantDocIDs.add(Integer.parseInt(docID));  //Storing the doc ids of user feedback
			
		}
		
		docScoreAll = indexer.getDocScore();
